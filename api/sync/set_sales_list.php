...
try {
    ...
    // insert query
    $sql = "INSERT INTO sales_list (invoice_no,product_id,name,amount,cost_amount,qty,price,profit,date,loading_id,action,cus_id,price_id,vat,value) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    $ql = $db->prepare($sql);
    $ql->execute(array($invoice, $pid, $name, $amount, $cost_amount, $qty, $price, $profit, $date, $load, 0, $cus, $price_id, $vat, $value));
